...
class GradeService:
    ...
    async def update_grade(self, db: AsyncSession, grade_id: int, grade_in: GradeUpdate) -> Type[Grade]:
        """更新成绩"""
        # 获取成绩记录
    ...
    async def publish_grade(self, db: AsyncSession, grade_id: int) -> Type[Grade]:
        """发布成绩"""
        grade = await db.get(Grade, grade_id)
        if not grade:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="成绩记录不存在")
    ...
    async def update_grade_rule(self, db: AsyncSession, rule_id: int, rule_in: GradeRuleUpdate) -> Type[GradeRule]:
        """更新成绩规则"""
